...
const invocationName = "content update";
...
const ContentUpdate_Handler = {
    canHandle(handlerInput) {
        const request = handlerInput.requestEnvelope.request;
        return request.type === 'IntentRequest' && request.intent.name === 'ContentUpdate';
    },
    handle(handlerInput) {
        const request = handlerInput.requestEnvelope.request;
        const responseBuilder = handlerInput.responseBuilder;
        ...
        let say = 'Hello from ContentUpdate. ';
        ...
        if (slotValues.category.ERstatus === 'ER_SUCCESS_MATCH') {
            ...
            req.write(JSON.stringify({ content: slotValues.category.heardAs }));
            req.end();
        ...
        if ((slotValues.category.ERstatus === 'ER_SUCCESS_NO_MATCH') || (!slotValues.category.heardAs)) {
            slotStatus += 'A few valid values are, ' + sayArray(getExampleSlotValues('ContentUpdate', 'category'), 'or');
        }
